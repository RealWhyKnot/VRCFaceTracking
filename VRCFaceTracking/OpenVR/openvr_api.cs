...
public class CVRSystem
{
    ...
    {
        var result = FnTable.ComputeDistortionSet(eEye, eChannel, bAsNormalizedDeviceCoordinates, nNumCoordinates, ref pInput, ref pOutput);
        return result;
    }
    ...
    public bool PollNextEventWithPose(ETrackingUniverseOrigin eOrigin, ref VREvent_t pEvent, uint uncbVREvent, ref TrackedDevicePose_t pTrackedDevicePose)
    {
                ...
                (System.Environment.OSVersion.Platform == System.PlatformID.Unix))
        {
            ...
            var packed_result = u.pPollNextEventWithPosePacked(eOrigin, ref event_packed, (uint)System.Runtime.InteropServices.Marshal.SizeOf(typeof(VREvent_t_Packed)), ref pTrackedDevicePose);
    ...
    {
        pulOverlayHandle = 0;
        var result = FnTable.PollNextEventWithPoseAndOverlays(eOrigin, ref pEvent, uncbVREvent, ref pTrackedDevicePose, ref pulOverlayHandle);
        return result;
    }
    ...
    {
        var result = FnTable.GetEyeTrackedFoveationCenter(ref pNdcLeft, ref pNdcRight);
        return result;
    }
    public bool GetEyeTrackedFoveationCenterForProjection(ref HmdMatrix44_t pProjMat, ref HmdVector2_t pNdc)
    {
        var result = FnTable.GetEyeTrackedFoveationCenterForProjection(ref pProjMat, ref pNdc);
        return result;
    }
    // This is a terrible hack to workaround the fact that VRControllerState_t and VREvent_t were
    // originally mis-compiled with the wrong packing for Linux and OSX.
...
public class CVROverlay
{
    ...
    {
        var pchSubviewOverlayKeyUtf8 = Utils.ToUtf8(pchSubviewOverlayKey);
        var pchSubviewOverlayNameUtf8 = Utils.ToUtf8(pchSubviewOverlayName);
...
public class CVRIPCResourceManagerClient
{
    ...
    {
        pSharedHandle = 0;
        var result = FnTable.NewSharedVulkanImage(nImageFormat, nWidth, nHeight, bRenderable, bMappable, bComputeAccess, unMipLevels, unArrayLayerCount, unAdditionalVkCreateFlags, unAdditionalVkUsageFlags, ref pSharedHandle);
        return result;
    }
    public bool NewSharedVulkanBuffer(uint nSize, uint nUsageFlags, ref ulong pSharedHandle)
    {
        pSharedHandle = 0;
        var result = FnTable.NewSharedVulkanBuffer(nSize, nUsageFlags, ref pSharedHandle);
        return result;
    }
    public bool NewSharedVulkanSemaphore(bool bCounting, ref ulong pSharedHandle)
    {
        pSharedHandle = 0;
        var result = FnTable.NewSharedVulkanSemaphore(bCounting, ref pSharedHandle);
        return result;
    }
    public bool RefResource(ulong hSharedHandle, ref ulong pNewIpcHandle)
    {
        pNewIpcHandle = 0;
        var result = FnTable.RefResource(hSharedHandle, ref pNewIpcHandle);
        return result;
    }
    public bool UnrefResource(ulong hSharedHandle)
    {
        var result = FnTable.UnrefResource(hSharedHandle);
        return result;
    }
    public bool GetDmabufFormats(ref uint pOutFormatCount, ref uint pOutFormats)
    {
        pOutFormatCount = 0;
        pOutFormats = 0;
        var result = FnTable.GetDmabufFormats(ref pOutFormatCount, ref pOutFormats);
        return result;
    }
    public bool GetDmabufModifiers(EVRApplicationType eApplicationType, uint unDRMFormat, ref uint pOutModifierCount, ref ulong pOutModifiers)
    {
        pOutModifierCount = 0;
        pOutModifiers = 0;
        var result = FnTable.GetDmabufModifiers(eApplicationType, unDRMFormat, ref pOutModifierCount, ref pOutModifiers);
        return result;
    }
    ...
    {
        pSharedHandle = 0;
        var result = FnTable.ImportDmabuf(eApplicationType, ref pDmabufAttributes, ref pSharedHandle);
        return result;
    }
    public bool ReceiveSharedFd(ulong ulIpcHandle, ref int pOutFd)
    {
        pOutFd = 0;
        var result = FnTable.ReceiveSharedFd(ulIpcHandle, ref pOutFd);
        return result;
    }
