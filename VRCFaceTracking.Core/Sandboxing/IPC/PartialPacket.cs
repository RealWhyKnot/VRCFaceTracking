...
public class PartialPacket : IpcPacket
{
    ...
    public static void DecodePacket(byte[] data, out byte[] packetData)
    {
        ...
        if ((packetPart & PACKET_PART_EOF_BIT) == PACKET_PART_EOF_BIT)
        {
            ...
            packetPart &= ~PACKET_PART_EOF_BIT;
    ...
    public static byte[][] SplitPacketIntoChunks(byte[] packetData, int mtu)
    {
        ...
        var partialPacketCount = (long)Math.Ceiling((double)packetData.Length / (mtu - PACKET_NON_DATA_SIZE_BYTES));
        ...
        for (uint i = 0; i < partialPacketCount; i++)
        {
            ...
            var packetSize = Math.Min((int)(bytesLeftToPack + PACKET_NON_DATA_SIZE_BYTES), mtu);
            var packetSizeNoOverhead = (int)Math.Max(packetSize - PACKET_NON_DATA_SIZE_BYTES, 0);
